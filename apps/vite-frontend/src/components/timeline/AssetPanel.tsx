...
export function AssetPanel({projectId, onSelectAsset}: AssetPanelProps) {
  ...
  return (
    <div className="w-64 bg-slate-800 border-l border-slate-700 flex flex-col h-full">
          ...
          {(['all', 'IMAGE', 'VIDEO', 'AUDIO'] as const).map((type) => (
            <button
              ...
              className={`flex-1 px-2 py-1 text-xs rounded ${
                typeFilter === type ? 'bg-purple-500 text-white' : 'bg-slate-700 text-slate-400 hover:bg-slate-600'
              }`}
            >
          ...
          <div className="grid grid-cols-2 gap-2">
            {assets.map((asset) => (
              ...
              >
                ...
                {/* 类型图标 */}
                <div className="absolute top-1 right-1 bg-black/50 rounded p-0.5">{getAssetIcon(asset.type)}</div>
              </div>
            ))}
          </div>
        )}
      ...
      {/* 底部提示 */}
      <div className="p-2 border-t border-slate-700 text-xs text-slate-500 text-center">点击或拖拽素材到时间线</div>
    </div>
  );
}
